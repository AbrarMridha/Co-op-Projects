...
class ImageProcessingApp:
    def __init__(self):
        ...
        btn_hist_stretch = Button(self.frame, text="Histogram Stretch", command=self.display_histogram_stretch,
                                  width=20)
        btn_hist_stretch.grid(row=0, column=1, padx=10, pady=10)
    ...
    # Apply histogram stretch and display
    def display_histogram_stretch(self):
        if self.image is not None:
            stretched_image = hf.histogram_stretching(self.image)
            self.display_image(stretched_image, "Histogram Stretched Image")
